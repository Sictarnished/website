...
if (isConfigured) {
  try {
    redis = new VercelKV({ url: kvRestApiUrl!, token: kvRestApiToken! });
  } catch (error) {
...
const getAccessToken = async () => {
  if (!isConfigured || !basicToken) return null;
...
const getLastSong = async (): Promise<SpotifyResponse> => {
  if (!redis) return { isListening: false, name: null };

  try {
    const lastSpotifySong = (await redis.get("last-spotify-song")) as SpotifyResponse | null;
    if (lastSpotifySong) return lastSpotifySong;
  } catch (error) {
...
const getSpotifyStatus = async (): Promise<SpotifyResponse> => {
  if (!isConfigured) return { isListening: false, name: null };

  const accessTokenResponse = await getAccessToken();
  const accessToken = accessTokenResponse?.access_token;

  if (!accessToken) return getLastSong();
  ...
  try {
    ...
    if (!response.ok || response.status === 204) return getLastSong();
    ...
    const spotifySchema = z.object({
      item: z.object({
        ...
        artists: z.array(
          z.object({ name: z.string(), external_urls: z.object({ spotify: z.string() }) })
        ),
      }),
    ...
    if (!validation.success) return getLastSong();
    ...
    const spotifyStatus: SpotifyResponse = {
      ...
      thumbnailUrl: parsed.item.album.images[1]?.url ?? parsed.item.album.images[0].url,
    ...
    if (redis) await redis.set("last-spotify-song", spotifyStatus).catch(console.error);
...
export async function GET() {
  if (!isConfigured) return NextResponse.json({ isListening: false, name: null });
  if (!redis) return NextResponse.json({ isListening: false, name: null });

  try {
    const cached = (await redis.get("spotify-cache")) as SpotifyResponse | null;
